  ...
  header('location:logout.php');
	...
	if (isset($_POST['submit'])) {
        ...
        if ($checkindate <  $cdate) {
            ...
            $sql = "SELECT * FROM tblbooking WHERE RoomId = :rid AND (CheckinDate BETWEEN :checkindate AND :checkoutdate OR CheckoutDate BETWEEN :checkindate AND :checkoutdate)";
            ...
            if ($rowCount > 0) {
                echo '<script>alert("There is already a booking for this room within the selected date range. Please choose different dates.")</script>';
            } else {
                // If no overlapping bookings found, proceed with insertion
                $sql = "INSERT INTO tblbooking(RoomId, BookingNumber, UserID, IDType, Gender, Address, CheckinDate, CheckoutDate,payment) VALUES(:rid, :booknum, :uid, :idtype, :gender, :address, :checkindate, :checkoutdate,'unpaid')";
                $query = $dbh->prepare($sql);
                $query->bindParam(':rid', $rid, PDO::PARAM_INT);
...
<html>
<head>
<title>Imagica Resort | Resort:: Book Room</title>
