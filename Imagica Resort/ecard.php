...
$invid = $_GET['invid'];
$sql = "SELECT tbleventbooking.BookingNumber, DATEDIFF(tbleventbooking.CheckoutDate, tbleventbooking.CheckinDate) as ddf, tbleventbooking.IDType, tbleventcategory.CategoryName, tbleventcategory.Price 
...
if(isset($_POST['submit']))
{
  ...
  $price=$_POST['price'];
  $invid=$_GET['invid'];
  $q1 = "INSERT INTO payment (Type,BookingNumber,cardno, cardholder, month, year, cvv,price) VALUES ('Event','$bookingNumber','$cardno', '$cardholder', '$month', '$year', '$cvv','$price')";
  $data = mysqli_query($con, $q1);  
  if($data)
  {
    echo '<script>alert("Payment has successfully")</script>';
    $q2="UPDATE tbleventbooking SET payment = 'Paid' WHERE BookingNumber=$bookingNumber";
    $data = mysqli_query($con, $q2);  
    echo "<script>window.location.href = 'eventinvoice.php?invid=" . $invid . "'</script>";
  }
  else
  {
