...
if(isset($_POST['submit']))
{
  ...
  $q1 = "INSERT INTO payment (cardno, cardholder, month, year, cvv,price) VALUES ('$cardno', '$cardholder', '$month', '$year', '$cvv','$price')";
...
if (isset($_GET['invid'])) {
    $vid = $_GET['invid'];
    $sql = "SELECT tblbooking.ID, tblcategory.Price, tblbooking.ID as tid FROM tblbooking JOIN tblcategory ON tblcategory.ID = tblbooking.RoomId WHERE tblbooking.ID=:vid";
    ...
    // Checking if the query returned a row
    if (!$row) {
        echo "No data found!";
...
<head>
  <meta charset="UTF-8">
  <title>CodePen -  Credit Card Form | Nothing4us </title>
  <link rel="stylesheet" href="css/card.css">
    ...
    <div class="card-form">
      ...
      <div class="card-form__inner">
        <div class="card-input">
          ...
          <input type="text" name="cardno" id="cardNumber" class="card-input__input" v-mask="generateCardNumberMask" v-model="cardNumber" v-on:focus="focusInput" v-on:blur="blurInput" data-ref="cardNumber" autocomplete="off">
        </div>
        <div class="card-input">
          <label for="cardName" class="card-input__label">Card Holders</label>
          <input type="text" name="cardholder" id="cardName" class="card-input__input" v-model="cardName" v-on:focus="focusInput" v-on:blur="blurInput" data-ref="cardName" autocomplete="off">
        </div>
        ...
        <div class="card-form__group">
          ...
          <div class="card-form__col -cvv">
            <div class="card-input">
              ...
              <input type="text" name="cvv" class="card-input__input" id="cardCvv" v-mask="'####'" maxlength="4" v-model="cardCvv" v-on:focus="flipCard(true)" v-on:blur="flipCard(false)" autocomplete="off">
            </div>
          </div>
        </div>
        <div class="card-input">
          <label for="cardName" class="card-input__label">Price</label>
          <input type="text" name="price"  class="card-input__input" <?php // echo $row->Price;?> require>
        </div>
        <button class="card-form__button" name="submit"> Payment  </button>
