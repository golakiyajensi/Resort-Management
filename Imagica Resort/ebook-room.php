  ...
  header('location:logout.php');
	...
	if (isset($_POST['submit'])) {
        ...
        if ($checkindate <  $cdate) {
            ...
            // Check if there are overlapping bookings for the selected room within the given date range
            $sql = "SELECT * FROM tbleventbooking WHERE EventId = :rid AND (CheckinDate BETWEEN :checkindate AND :checkoutdate OR CheckoutDate BETWEEN :checkindate AND :checkoutdate)";
            ...
            if ($rowCount > 0) {
                echo '<script>alert("There is already a booking for this Event within the selected date range. Please choose different dates.")</script>';
            } else {
                // If no overlapping bookings found, proceed with insertion
                $sql="insert into tbleventbooking(EventId,BookingNumber,UserID,IDType,Address,CheckinDate,CheckoutDate,payment)values(:rid,:booknum,:uid,:idtype,:address,:checkindate,:checkoutdate,'unpaid')";
                $query = $dbh->prepare($sql);
                $query->bindParam(':rid', $rid, PDO::PARAM_INT);
               ...
               // $query->bindParam(':gender', $gender, PDO::PARAM_STR);
                ...
                if ($LastInsertId > 0) {
                    echo '<script>alert("Your Event has been booked successfully. Booking Number is ' . $booknum . '")</script>';
...
<html>
<head>
<title>Imagica Resort | Resort:: Book Event</title>
